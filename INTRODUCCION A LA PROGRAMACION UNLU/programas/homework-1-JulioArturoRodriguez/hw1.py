...
# Nombre y Apellido: julio arturo rodriguez
# DNI: 40631119
################################################################################################
...
def valorAbsoluto(x):
    r = f"El valor absoluto de {x} es {abs(x)}"
    return r
...
def contarLetrasNombre(x):
    N = len(x)
    a = f"El nombre {x} tiene {N} letras."
    return a
...
def cambioDolar(pesos):
    cambio = 90
    dolares = round(pesos / cambio)
    return f"Si me das {pesos} pesos, te doy {dolares} dolares"
